 ...
 */
const { maxLengthURL, minPortInteger, maxPortInteger } = require('../config');
...
const checkSchemeChars = function checkSchemeChars(scheme, len) {
  if (!is(String, scheme)) {
    const error = new URIError('scheme must be a string');
  ...
  if (schemeLen <= 0) {
    const error = new URIError('scheme cannot be empty');
    error.code = 'URI_INVALID_SCHEME';
    throw error;
  }
...
const checkURI = function checkURI(uri, { sitemap } = {}) {
  ...
  if (exists(authority)) {
    ...
    if (exists(port) && int(port, { ge: minPortInteger, le: maxPortInteger }) === undefined) {
      const error = new URIError(`port must be an integer between ${minPortInteger}-${maxPortInteger}, got '${port}'`);
 ...
 *
 * Same behavior than checkHttpSitemapURL except scheme can be http/HTTP or https/HTTPS.
 *
 * @param  {String} uri
 * @return {Object}
 * @throws {URIError}

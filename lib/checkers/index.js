 ...
 * - checkPathqf({ type, string, sitemap } = {}) -> undefined throws URIError
 ...
 *
 * Check an entity in an URL found from a string at a specific index has a valid
 * escape encoding following this char.
 *
 * @param  {String} string
 * @param  {Number} index
 * @param  {Number} stringLen
...
const checkSitemapEncoding = function checkSitemapEncoding(string, index, stringLen) {
  ...
  if (string[i] === '&') {
    ...
    for (let j = 0; j < escapeCodesKeysLen; j += 1) {
      ...
      if (i + codeLen <= stringLen && code === string.substring(i, i + codeLen)) {
    ...
    if (!exists(escapeOffset)) {
      const error = new URIError(`entity '${string[i]}' must be escaped`);
      error.code = 'URI_INVALID_SITEMAP_CHAR';
      throw error;
    } else {
      offset = escapeOffset;
    }
  } else if (entitiesKeys.includes(string[i])) {
    // then if a char must be escaped
    const error = new URIError(`entity '${string[i]}' must be escaped`);
    error.code = 'URI_INVALID_SITEMAP_CHAR';
    throw error;
  }
 ...
 * @func checkPathqf
 *
 * Check a string has valid characters following path, query,
 ...
 * @throws {URIError}
 */
const checkPathqf = function checkPathqf({ type, string, sitemap } = {}) {
  if (!['path', 'query', 'fragment'].includes(type)) {
  ...
  if (sitemap) {
    checkCharFunc = type === 'path' ? isSitemapPathChar : isSitemapQueryOrFragmentChar;
  } else {
    checkCharFunc = type === 'path' ? isPathChar : isQueryOrFragmentChar;
  }
...
const checkURI = function checkURI(uri, { sitemap } = {}) {
  ...
  if (exists(authority)) {
    ...
    for (let i = 0; i < userinfoLen; i += 1) {
      if ((sitemap && !isSitemapUserinfoChar(userinfo[i])) || !isUserinfoChar(userinfo[i])) {
  ...
  checkPathqf({ sitemap, type: 'path', string: path });
  checkPathqf({ sitemap, type: 'query', string: query });
  checkPathqf({ sitemap, type: 'fragment', string: fragment });
...
console.log(checkURI('http://User:Pass@[2001:db8:a88:85a3::ac1f:8001]/index.html'));

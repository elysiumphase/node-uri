...
const checkPathqf = function checkPathqf({ type, string, sitemap } = {}) {
  ...
  const checkCharFunc = type === 'path' ? isPathChar : isQueryOrFragmentChar;
...
const checkURI = function checkURI(uri, { sitemap } = {}) {
  ...
  checkPathqf({ sitemap, component: 'path', string: path });
  checkPathqf({ sitemap, component: 'query', string: query });
  checkPathqf({ sitemap, component: 'fragment', string: fragment });
...
console.log(checkURI('http://[2001:db8:a88:85a3::ac1f:8001]/index.html'));

...
const encodeURIComponentString = function encodeURIComponentString(
  ...
  for (let i = 0; i < componentToEncodeLen; i += 1) {
    ...
    if (i < componentToEncodeLen) {
      switch (type) {
        case 'userinfo':
          uricomponent += isUserinfoCharToEncode(componentToEncode[i])
            ? encodeURIComponent(componentToEncode[i])
            : componentToEncode[i];
          break;
        case 'path':
          uricomponent += isPathCharToEncode(componentToEncode[i])
            ? encodeURIComponent(componentToEncode[i])
            : componentToEncode[i];
          break;
        case 'query':
        case 'fragment':
          uricomponent += isQueryOrFragmentCharToEncode(componentToEncode[i])
            ? encodeURIComponent(componentToEncode[i])
            : componentToEncode[i];
          break;
        default:
          uricomponent += encodeURIComponent(componentToEncode[i]);
      }
    }
  }
...
const encodeURIString = function encodeURIString(uri, { web, sitemap, lowercase } = {}) {
  ...
  if ((web === true || sitemap === true) && !isIP(host) && !isDomain(host)) {
  ...
  // if authority:
  // - encode non ASCII characters for userinfo and following authority;
  // - use punycoded host.
  if (exists(authority)) {
